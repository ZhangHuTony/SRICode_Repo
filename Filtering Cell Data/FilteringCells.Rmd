...
melanocyte6_Dataframe = read.csv("Data/melanocyte6hrs.csv")
```
...
```{r}
Barbie_Rescaled_Df = rescalePosition(Filt6_BarbieMelanoma_Dataframe)
```
...
kenRescaledDf = rescalePosition(Filt6_KenMelanoma_Dataframe)
```

```{r}
melanocyteRescaledDf = rescalePosition(melanocyte6_Dataframe)
```


```{r}
hex <- hexbin(Barbie_Rescaled_Df$Centered_Position.X, Barbie_Rescaled_Df$Centered_Position.Y, xbins = 10)
plot(hex,colramp = colorRampPalette(hcl.colors(12)))

hex <- hexbin(kenRescaledDf$Centered_Position.X, kenRescaledDf$Centered_Position.Y)
plot(hex,colramp = colorRampPalette(hcl.colors(12)))

melanocyteHex <- hexbin(melanocyteRescaledDf$Centered_Position.X, melanocyteRescaledDf$Centered_Position.Y)
plot(melanocyteHex,colramp = colorRampPalette(hcl.colors(12)))

```

```{r}
ggplot(Barbie_Df_25, aes(x = Position.X, y = Position.Y, fill = Stopped.Tracking)) +
  geom_tile(width = 20, height = 20) +
  scale_fill_gradient(low = "blue", high = "red") +
  coord_fixed()+
    labs(x = "X Position", y = "Y Position", title = "Stopped Tracking Heat Map") 

```
```{r}
library(latticeExtra)

levelplot(Dry.Mass ~ Centered_Position.X * Centered_Position.Y, kenRescaledDf, 
