...
```{r}
model2 <- glm(left ~ duration+Dry.Mass+Distance_From_Center, data = distanceFromCenterDf, family = binomial, subset = Distance_From_Center < mean(Distance_From_Center))
summary(model2)
```
...
getNumberOfCellsWithin <- function(Df, distance){
  ...
  # Creating a new dataframe by selecting specific columns from scaledDf
  frame_df_positions <- frame1df[selected_columns]
