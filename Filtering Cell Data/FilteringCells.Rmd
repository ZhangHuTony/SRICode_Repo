...
# Assuming scaledDf is your original dataframe
# Columns to be included in the new dataframe
selected_columns <- c("Centered_Position.X", "Centered_Position.Y", "Dry.Mass")

# Creating a new dataframe by selecting specific columns from scaledDf
new_df <- distanceFromCenterDf[selected_columns]

data1 = new_df %>% 
  rename(
    X = Centered_Position.X,
    Y = Centered_Position.Y,
    Z = Dry.Mass
    )
...
ggplot(data1, aes(x = X, y = Y, fill = Z)) +
  ...
  labs(x = "Centered_Position.X", y = "Centered_Position.Y", fill = "Dry.Mass")
```
