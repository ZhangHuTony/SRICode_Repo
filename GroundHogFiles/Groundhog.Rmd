...
author: "Montana Ferita & Tony Zhang"
date: "2024-02-01"
output:
...
**Punxsutawney Phil is nervous about his big day and he keeps practicing going inside and outside of his hole. Suppose the probability he leaves his hole is 0.4 and the probability he goes back in his hole is 0.3. What are the conditional probabilities?** 
- Pr($I_t|O_{t-1}$): 0.4 
- Pr($O_t|I_{t-1}$): 0.3
...
**Find the long-term probability Punxsutawney is outside.** 
- $\\O^* = \frac{4}{7}$
...
#List to keep track of each loop to get an average
timeOutsideList <-numeric(9999)
...
#Does it 9999 times to get an average
for(i in 1:9999){
  ...
  timesOutsideList = sum(statesList==2)
}
...
```{r}
plot(statesList)
print(mean(timesOutsideList))
```
Result is very close to 4/7 so yes it does agree with the long term probability
